...
// Simple in-memory rate limiting (max 5 submissions per 10 minutes per IP)
  ...
  try {
    ...
    if (isRateLimited(ip)) {
      return NextResponse.json(
        { success: false, message: 'Too many requests. Please try again later or contact us on WhatsApp.' },
        { status: 429 }
      );
    }
    ...
    const pageUrl = sanitize(body.page_url || req.headers.get('referer') || 'https://www.growthdigitech.com/');
    ...
    if (!validatePhone(phone)) {
      return NextResponse.json({ success: false, message: 'Please enter a valid phone or WhatsApp number.' }, { status: 400 });
    }
    ...
    if (!smtpUsername || !smtpPassword) {
      ...
      return NextResponse.json({ success: false, message: 'Unable to send your enquiry. Please try again.' }, { status: 500 });
    }
    ...
    // Phone formatting for Tel & WhatsApp links
...
${message}
    ...
    // Mail options: setFrom authenticated address, replyTo visitor's email
    const mailOptions = {
    ...
    // Optional Visitor Acknowledgement Email
